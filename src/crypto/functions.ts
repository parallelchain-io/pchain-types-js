...
*/
export const base64Tobase64url = (stringB64: string) => stringB64.replace(/\//g, '_').replace(/\+/g, '-').replace(/=/g, '');
...
export const bytesToBigInt = (bytes: Buffer) => {
    if(bytes.byteLength == 8){
        ...
        return num1.add(num2.add(num3.add(num4.add(num5.add(num6.add(num7.add(num8))))))).toString()
    }
    return 0;
}
...
export const generateSha256Hash = (message: Uint8Array) => {
    let h1 = createHash('sha256').update(message);
    let h2 = h1.digest();
