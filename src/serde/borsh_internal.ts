...
/* eslint-disable */
/* README
this is a duplicate of v0.7.0 of borsh-ts/index.ts in the borsh-js package,
except for areas marked with MODIFICATION
permalink ref: 
https://github.com/near/borsh-js/blob/431f2b403b5d417152ae3b5a1201493de0724304/borsh-ts/index.ts
*/

// MODIFICATION
// use pchain-types-js Option class
...
export class BinaryWriter {
  ...
  // MODIFICATION
  // support type for boolean
  // declared using "boolean" string
  ...
  try {
    ...
    if (typeof fieldType === "string") {
      ...
      switch (fieldType.kind) {
        // MODIFICATION
        // Support type for variable length bytearray
        // declared using: type: { kind: "bytearray" }
        case "bytearray": {
        ...
        case "option": {
          // MODIFICATION
          // read option.value when serializaing
          if (value.value == null) {
            writer.writeU8(0);
          } else {
            writer.writeU8(1);
        ...
        // MODIFICATION
        // Support type for set
        // declared using: type: { kind: "set": type: <type> }
        case "set": {
          writer.writeU32(value.size); // set size
          value.forEach((val) => {
            serializeField(schema, fieldName, val, fieldType.type, writer);
          });
          break;
        }
        //
...
function deserializeField(
  ...
  try {
    ...
    // Support type for variable length bytearray
    // declared using: type: { kind: "bytearray" }
    if (fieldType.kind === "bytearray") {
      const length = reader.readU32();
      const byteArray = reader.readFixedArray(length);
      return byteArray;
    }
    //
    // END MODIFICATION
    ...
    // Support type for set
    // declared using: type: { kind: "set": type: <type> }
    if (fieldType.kind === "set") {
      const set = new Set();
      const length = reader.readU32();
      for (let i = 0; i < length; i++) {
        const item = deserializeField(
          schema,
          fieldName,
          fieldType.type,
          reader
    ...
    if (fieldType.kind === "option") {
      ...
      // MODIFICATION
      // deserialize option with wrapper class
      if (option) {
        return new Option({
          value: deserializeField(schema, fieldName, fieldType.type, reader),
        });
      }
